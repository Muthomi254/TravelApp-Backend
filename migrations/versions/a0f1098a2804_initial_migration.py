...
Revision ID: a0f1098a2804
Revises: 
Create Date: 2024-02-20 09:30:30.005045
...
def upgrade():
    ...
    op.create_table('reservation_accomodation',
    sa.Column('id', sa.Integer(), nullable=False),
    ...
    op.create_table('reservation_travels',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('people_included', sa.SmallInteger(), nullable=False),
    sa.Column('date', sa.DateTime(), server_default=sa.text('(CURRENT_TIMESTAMP)'), nullable=True),
    ...
    op.create_table('Reviews_accomodation',
    sa.Column('id', sa.Integer(), nullable=False),
    ...
    op.create_table('reviews_travels',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('rating', sa.Float(), nullable=False),
    sa.Column('review', sa.Text(), nullable=True),
    sa.Column('created_at', sa.DateTime(), server_default=sa.text('(CURRENT_TIMESTAMP)'), nullable=True),
    ...
    sa.Column('review_count', sa.Integer(), nullable=True),
    sa.ForeignKeyConstraint(['travel_id'], ['travelling_services.id'], ),
    sa.ForeignKeyConstraint(['user_id'], ['users.id'], ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id')
    )
    ...
    sa.Column('travelling_service_id', sa.String(length=12), nullable=True),
    sa.ForeignKeyConstraint(['travelling_reservation_id'], ['reservation_travels.id'], ),
    sa.ForeignKeyConstraint(['travelling_service_id'], ['travelling_services.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
...
def downgrade():
    ...
    op.drop_table('reviews_travels')
    op.drop_table('accomodation_bookings')
    op.drop_table('Reviews_accomodation')
    op.drop_table('travelling_services')
    op.drop_table('reservation_travels')
    op.drop_table('reservation_accomodation')
